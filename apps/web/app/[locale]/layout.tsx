...
export default async function RootLayout({
  ...
  const [signedIn, restorable] = await Promise.all([
    hasSessionCookie(),
    canRestoreSession(),
  ]);
  ...
  return (
    <html lang={LOCALE_TAG[locale]} className={spaceGrotesk.variable}>
      <body>
        {/* Wraps EVERYTHING, not just the main slot. The nav and the footer are
            client components as well, and with the provider around `children`
            only they fell back to English on a Spanish page — the fallback
            working exactly as designed, in the wrong place. */}
